  ...
  return <div className="page settings-page">
    ...
    <section className="settings-sheet">
      ...
      {tab === "llm" && <>
        ...
        <Field label={`实时字幕首 token 超时 · ${(s.llm.timeout_milliseconds / 1000).toFixed(1)} 秒`} hint="仅限制实时字幕等待首 token；收到内容后按流空闲时间判断"><input type="range" min="1000" max="5000" step="250" value={Math.min(s.llm.timeout_milliseconds, 5000)} onChange={e => update({ ...s, llm: { ...s.llm, timeout_milliseconds: Number(e.target.value) } })} /></Field>
